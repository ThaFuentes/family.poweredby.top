...
- Password vault (`/vault`): compact name/site cards. Add and each login open in the iframe sheet overlay — no forms sitting on the list. Sheet has username, password (Show/Copy), Open site, edit, and who-can-see. Encrypted at rest. Unlock with Family OS username + password. Kids never see the vault.
