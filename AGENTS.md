...
- Password vault (`/vault`): create the login first (title, URL, username, password, purpose, details). Sharing is after: just you, whole household, or named people with an optional time limit — change it anytime. Encrypted at rest. Opening secrets requires the signed-in person's **username and password**. Locks again after 10 minutes. Do not log plaintext vault fields. Kids never see the vault.
