...
- Password vault (`/vault`): one screen after unlock. Each login is a card with title, username, password (Show/Copy), site, details, edit, and who-can-see. **Open site** is a button — the card is not a link. Create first, share after. Encrypted at rest. Unlock with Family OS username + password. Saving a new login does not require that extra unlock. Idle lock after 45 minutes. Kids never see the vault.
