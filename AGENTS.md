...
- Phone is the main client. Do not ship a reduced mobile app. Inventory (what’s in the house) and Basket (the store list) are both first-class. Home tiles + More expose tools, vehicles, house, notes, records, due — same features as the desktop side nav.
...
- Scan on an open vehicle, tool, or house is for that thing. Banner: **SCAN FOR X VEHICLE / EQUIPMENT**. Any UPC attaches as equipment, with **Undo**. VIN on an open vehicle posts to that vehicle. VIN with **Add a vehicle** open creates a new one. If NHTSA differs from what we already stored, show the diff and ask.
